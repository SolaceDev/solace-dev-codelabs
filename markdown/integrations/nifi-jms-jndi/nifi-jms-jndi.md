...
![Sequence Diagram](img/nifisequence.png)
...
This is a demostration of an approach for publishing and consuming messages from a Java Messaging Service (JMS) ControllerService in NiFi processors. The full code is freely available on Github as part of this project in [src/nifi-jms-jndi](https://github.com/SolaceLabs/solace-integration-guides/blob/master/src/nifi-jms-jndi).
...
For reference, the CLI commands in the following sections are from SolOS version 7.2 but will generally be forward compatible. For more details related to Solace Event Broker CLI see [Solace Command Line Interface Reference](https://docs.solace.com/Solace-CLI/Using-Solace-CLI.htm) . Wherever possible, default values will be used to minimize the required configuration. The CLI commands listed also assume that the CLI user has a Global Access Level set to Admin. For details on CLI access levels please see [Solace Documentation](https://docs.solace.com/Configuring-and-Managing/CLI-User-Access-Levels.htm)  section "User Authentication and Authorization".
...
*    [src/nifi-jms-jndi](https://github.com/SolaceLabs/solace-integration-guides/blob/master/src/nifi-jms-jndi)
