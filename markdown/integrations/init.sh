...
# local variables
codelab_markdown_filename="$CODELAB_NAME/$CODELAB_NAME.md"
codelab_package_json_filename="$CODELAB_NAME/package.json"
markdown_template="../template/markdown-iguide.template"
package_json_template="../template/package.json"
#in MacOS sed creates a backup file if zero length extension is not specefied e.g. ''
backup_md="$codelab_markdown_filename-e"
backup_package_json="$CODELAB_NAME/package.json-e"
...
mkdir $CODELAB_NAME
rsync -a --exclude=markdown.template ../template/* $CODELAB_NAME/

# rename markdown template file 
mv $CODELAB_NAME/markdown-iguide.template $codelab_markdown_filename
...
# Modify the export path for the codelabs directory in the export.sh script
sed -i \
  -e "s/..\/..\/codelabs/..\/..\/..\/codelabs/g" \
  $CODELAB_NAME/export.sh
...
if [ -f "$CODELAB_NAME/export.sh-e" ]; then
  rm $CODELAB_NAME/export.sh-e
fi

echo "Markdown file created! Find it at $PWD/$CODELAB_NAME"
